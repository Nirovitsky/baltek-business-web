...
>(
  (
    ...
    if (isMobile) {
      return null // Mobile handled separately
    }
    ...
    return (
      <div
        ...
        className={cn(
          ...
          "shrink-0 transition-[width] ease-linear",
          "relative h-svh bg-transparent",
          className
        )}
        {...props}
      >
        <div className="h-full w-full bg-sidebar border-r">
          {children}
        </div>
      </div>
    )
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "peer-data-[state=expanded]:ml-[--sidebar-width]",
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn(
        "duration-200 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opa] ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0 group-data-[collapsible=icon]/sidebar-wrapper:opacity-0",
        className
      )}
      {...props}
    />
...
// Placeholder for TooltipContent (we'll handle tooltips separately)
